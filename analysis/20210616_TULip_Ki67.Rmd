...
```{r plot_curves, fig.height=8, fig.width=10, fig.cap = "*Power curves for detection of an additional 20% reduction in Ki67 due to a combination treatment of L+TU in comparison to Letrazole alone. Panels represent equal samples sizes in both treatment groups (L+TU=L), having an extra 50% of participants in the combination group (L+TU = 1.5L), or having double the number of particiapnts in the combined treatment group (L+TU = 2L). Error bars indicate a 95% confidence interval around the power estimates*"}
sim_res %>% 
  mutate(
